...
def inflate_obstacles(grid_map, robot_radius, cell_size):
    """
    Inflate obstacles on the grid map based on the robot's radius.

    :param grid_map: 2D numpy array of the grid map
    :param robot_radius: radius of the robot in meters
    :param cell_size: size of each cell in meters
    :return: new grid map with inflated obstacles
    """
    # Calculate the number of cells to inflate based on the robot's radius
    inflation_radius = int(math.ceil(robot_radius / cell_size))
    
    # Create a structuring element for dilation (circular)
